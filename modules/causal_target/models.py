...
3. ``CausalTargetResult`` — a protein evaluated as a potential target
...
CausalTarget applies Pearl's do-calculus to distinguish genuine
causal targets from correlational bystanders.  The
``CausalTargetResult.is_causal_target`` flag captures this
...
class GraphNode(BaseModel):
    ...
    source: str = Field("", description="Database source (DisGeNET, OpenTargets, etc.)")
...
class DiseaseGraph(BaseModel):
    ...
    Assembled from multiple databases (DisGeNET, Open Targets,
...
class CausalTargetResult(BaseModel):
...
class PipelineJob(BaseModel):
    ...
    candidates_per_target: int = Field(1000, ge=10)
...
class PipelineResult(BaseModel):
    """Complete output of a CausalTarget pipeline run.
    ...
    causal_targets: list[CausalTargetResult] = Field(default_factory=list)
